...
writer.add_text('shuffle', str(args.shuffle))
writer.close()

net = TestDBP(**cfg['DBP_info'], Fs=cfg['Rs']*2e9)
...
train_data = MyDataset(cfg['train_path'], Nch=[cfg['Nch']], Rs=[cfg['Rs']], Pch=cfg['train_pch'], Nmodes=cfg['DBP_info']['Nmodes'],
...
train_loader = DataLoader(train_data, batch_size=cfg['batch_size'], shuffle=args.shuffle, drop_last=True)
print('data batchs:', len(train_loader))


test_info = {'Nch':cfg['Nch'], 'Rs': cfg['Rs'], 'Pch':cfg['test_pch'], 'Nmodes':cfg['DBP_info']['Nmodes']}
...
if cfg['Nmodes'] == 1:
    ...
    Pchs = range(-3, 8)
Qp = [Test(net, Pch=pch, Nch=cfg['Nch'], Rs=cfg['Rs'], Nmodes=cfg['DBP_info']['Nmodes'], test_path=cfg['test_path'], ber_discard=20000, Nsymb=80000)['Qsq'] for pch in Pchs]
print('Q factor:', Qp)
writer = SummaryWriter(log_path)
