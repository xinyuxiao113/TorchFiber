...
import src.JaxSimulation.operator as Op
...
def frame(y, taps, sps, rtap=None):
    y_pad = jnp.pad(y, mimozerodelaypads(taps=taps, sps=sps, rtap=rtap))  # type: ignore
    yf = jnp.array(Op.frame(y_pad, taps, sps))
    return yf
...
def iterate(update: UpdateFn,
    ...
    return steps[-1], Op.scan(lambda c, xs: update(xs[0], c, xs[1:]), state, xs, jit_device=device)
...
def cpane_ekf(train: Union[bool, Schedule] = False,
              ...
              R: complex =1e-2 + 0j,
              akf: bool = True,
              const: Array = QAM(16).const()) -> AdaptiveFilter:
