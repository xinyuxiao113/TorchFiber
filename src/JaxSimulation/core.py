...
class MySignal:
    ...
    t: SigTime=struct.field(pytree_node=False)      # (start,stop,sps)      
...
def wrap_signal(dataset):
    ...
    x = MySignal(dataset.x, SigTime(0,0,1), Fs/sps, power, Fi, Nch)
    y = MySignal(dataset.y, SigTime(0,0,sps),   Fs, power, Fi, Nch)
    return y,x


def align_signal(x, t:SigTime)->tuple:
    '''
    Align signal.
    Input:
        x, t
...
def get_MySignal(val, a):
    return MySignal(val=val, t=SigTime(0,0,a['sps']), Fs=a['samplerate'],power= a['power'], Fi=a['carrier_frequency'], Nch=a['Nch'])
...
def conv1d_slicer(taps, rtap=None, stride=1, mode='valid'):
    def slicer(signal: MySignal) -> MySignal:
        x = signal.val
        xt = signal.t
        ...
        return MySignal(val=x, Fs=signal.Fs, t=zt, power=signal.power, Fi=signal.Fi, Nch=signal.Nch)
    return slicer
