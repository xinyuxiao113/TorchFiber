...
def P(x: Array, axis:int=0)->Array:
    '''
...
def exp_integral(z: float, alpha:float = 4.605170185988092e-05, span_length:float=80e3) -> jax.Array:
    '''
...
def Leff(z: float, dz: float, alpha: float=4.605170185988092e-05, span_length: float=80e3) -> jax.Array:
    '''
       Optical Power integral along z.
    Input:
        z: Distance [m]
...
def dispersion_kernel(dz:float, dtaps:int, Fs:int, beta2:float=-2.1044895291667417e-26, beta1:float=0, domain='time') -> Array:
    ''' 
...
def dispersion_time_kernel(dz:float, dtaps:int, Fs:int, beta2:float=-2.1044895291667417e-26, beta1=0, domain='time', steps=8):
...
def pretrain_kernel(dz:float, dtaps:int, Fs:int, beta2:float=-2.1044895291667417e-26, beta1:float=0.0, domain:str='time', steps:int=8,pre_train_steps:int=200) -> jax.Array:
    Dtaps = (dtaps-1)*steps+1
    k = (Dtaps - dtaps)//2
    h = dispersion_kernel(dz, Dtaps, Fs, beta2, beta1, domain)[k:Dtaps-k]
