...
class ComplexLinear(nn.Module):
    ...
    def forward(self, x):
        return torch.complex(self.real(x.real) - self.imag(x.imag),
                             self.imag(x.real) + self.real(x.imag))
...
class ComplexLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, **kwargs):
        ...
        self.LSTM = nn.LSTM(input_size*2, hidden_size*2, num_layers, batch_first=True, **kwargs)
    ...
    def init_carry(self, batch, device):
        h_zeros = torch.zeros(self.num_layers, batch, self.hidden_size*2, dtype=torch.float32, device=device)
        c_zeros = torch.zeros(self.num_layers, batch, self.hidden_size*2, dtype=torch.float32, device=device)
        hx = (h_zeros, c_zeros)
        return hx
...
class ComplexGRU(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, **kwargs):
        ...
        self.hidden_size = hidden_size
        self.num_directions = 1
        self.GRU = nn.GRU(input_size*2, hidden_size*2, num_layers, batch_first=True, **kwargs)
...
class ComplexConv1d(nn.Module):
    ...
    '''
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=False, groups=1, init='xavier'):
        super(ComplexConv1d, self).__init__()
        self.conv1d_r = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias, groups=groups)
        self.conv1d_i = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias, groups=groups)
        if init == 'xavier':
