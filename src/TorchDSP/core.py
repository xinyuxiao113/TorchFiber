...
class TorchTime:
    ...
    def conv1d_t(self, taps, stride, rtap=None, mode='valid'):
        ...
        Output:
            None
        '''
        assert self.sps >= stride, f'sps of input SigTime must be >= stride: {stride}, got {self.sps} instead'
        if rtap is None: rtap = (taps - 1) // 2
...
class TorchInput:
    ...
    '''
    def __init__(self, signal_input: TorchSignal, signal_output: TorchSignal, task_info: torch.Tensor=torch.ones(4)):
